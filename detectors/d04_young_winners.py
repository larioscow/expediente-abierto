...
SELECT file_year,
  ...
  count(*) FILTER (edad_dias < 365 AND importe >= {MIN_IMPORTE} AND (moneda_drc='MXN' OR moneda_drc IS NULL)) AS jovenes_y_grandes,
  round(sum(importe) FILTER (edad_dias < 365 AND (moneda_drc='MXN' OR moneda_drc IS NULL)) / 1e6, 1) AS monto_a_menores_1a_mxn_m
...
FROM aged
WHERE edad_dias < 365 AND importe >= {MIN_IMPORTE} AND (moneda_drc='MXN' OR moneda_drc IS NULL)
